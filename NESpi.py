...
class NEScontroller:
	...
	def exec(self):
		...
		if GPIO.input(self.data):
			#GPIO.output(18, 1)
			self.uinput.write(e.EV_KEY, e.KEY_J, 0) #2 hold
			#print ("---A NOT pressed")
		else:
			#GPIO.output(18, 0)
			self.uinput.write(e.EV_KEY, e.KEY_J, 1)
			#print ("---A IS pressed")
		self.__clock() #B
		if GPIO.input(self.data):
			self.uinput.write(e.EV_KEY, e.KEY_H, 0)
		else:
			self.uinput.write(e.EV_KEY, e.KEY_H, 1)
		self.__clock() #SELECT
		if GPIO.input(self.data):
			self.uinput.write(e.EV_KEY, e.KEY_F, 0)
		else:
			self.uinput.write(e.EV_KEY, e.KEY_F, 1)
		self.__clock() #START
		if GPIO.input(self.data):
			self.uinput.write(e.EV_KEY, e.KEY_G, 0)
		else:
			self.uinput.write(e.EV_KEY, e.KEY_G, 1)
		self.__clock() #UP
		if GPIO.input(self.data):
			self.uinput.write(e.EV_KEY, e.KEY_W, 0)
		else:
			self.uinput.write(e.EV_KEY, e.KEY_W, 1)
		self.__clock() #DOWN
		if GPIO.input(self.data):
			self.uinput.write(e.EV_KEY, e.KEY_S, 0)
		else:
			self.uinput.write(e.EV_KEY, e.KEY_S, 1)
		self.__clock() #LEFT
		if GPIO.input(self.data):
			self.uinput.write(e.EV_KEY, e.KEY_A, 0)
		else:
			self.uinput.write(e.EV_KEY, e.KEY_A, 1)
		self.__clock() #RIGT
		if GPIO.input(self.data):
			self.uinput.write(e.EV_KEY, e.KEY_D, 0)
		else:
			self.uinput.write(e.EV_KEY, e.KEY_D, 1)
		self.__clock()
		self.uinput.syn()
...
def getUInput():
	global e
	cap={e.EV_KEY :
		[e.KEY_A, e.KEY_D, e.KEY_F, e.KEY_G, e.KEY_H, e.KEY_J, e.KEY_S, e.KEY_W]}
	'''
